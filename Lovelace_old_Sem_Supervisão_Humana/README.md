...
### Para modificação (estrutura atual)

Use a estrutura deste repositório para editar dados, scripts e treinar a LoRA. Este modo pressupõe que voce vai alterar arquivos em `data/`, `scripts/` e `model/`.

### Uso simplificado (Windows .exe, standalone)

Gera um executavel que roda sem Ollama e sem Python instalado.
Observacao: o .exe fica muito grande porque inclui o runtime do Python e o modelo base.

Pre-requisitos (para quem gera o .exe):
- Baixar o modelo base localmente (vicuna-7b-v1.5).
- Ter GPU ou aceitar execucao lenta em CPU.
- `pip install -r requirements.txt`
...
model/
  base/
    vicuna-7b-v1.5/   # pesos do modelo base
  ada-lovelace-lora/  # LoRA treinada
```
...
pyinstaller --onefile scripts/ada_lovelace_standalone.py --name ada-lovelace ^
  --add-data "model\\ada-lovelace-lora;model\\ada-lovelace-lora" ^
  --add-data "model\\base\\vicuna-7b-v1.5;model\\base\\vicuna-7b-v1.5"
```

Executar:

```bash
dist\ada-lovelace.exe "Explique a diferença entre prontuário funcional e social."
```

Dicas:
- Se preferir manter os pesos fora do .exe, coloque a pasta `model/` ao lado do executavel e use:
  - `set BASE_MODEL_PATH=.\model\base\vicuna-7b-v1.5`
  - `set LORA_PATH=.\model\ada-lovelace-lora`
- Se o PyInstaller reclamar de imports, adicione `--collect-all transformers --collect-all peft`.
...
│   ├── ada_lovelace_cli.py        # Launcher simples para empacotar em .exe (Windows)
