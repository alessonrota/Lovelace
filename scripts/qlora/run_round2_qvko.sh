...
RUN_ID="${RUN_ID:-qlora_r2_15cls_qvko_v1}"
RUN_DIR="data/processed/qlora-qwen14b/${RUN_ID}"
COPY_OCR_FROM="${COPY_OCR_FROM:-data/processed/qlora-qwen14b/qlora_r1_15cls_v2}"
...
QLORA_CONFIG="${QLORA_CONFIG:-configs/qlora/qlora_qwen14b_r2_qvko_lowmem.yaml}"
SKIP_AUTO_EVAL="${SKIP_AUTO_EVAL:-1}"
...
COPY_OCR_FLAG=()
if [ -n "${COPY_OCR_FROM}" ]; then
  COPY_OCR_FLAG=(--copy-ocr-from-run "${COPY_OCR_FROM}")
fi

SKIP_EVAL_FLAG=()
if [ "${SKIP_AUTO_EVAL}" = "1" ]; then
  SKIP_EVAL_FLAG=(--skip-auto-eval)
fi
