...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Manual evaluation of trained QLoRA adapter with stable inference controls.")
    p.add_argument(
        "--run-root",
        type=Path,
        default=Path("data/processed/qlora-qwen14b/qlora_r1_15cls_v2"),
    )
    ...
    p.add_argument("--splits", default="eval_legacy_93,val_balanced,test_holdout_balanced")
...
def main() -> int:
    ...
    if out_dir is None:
        ...
        out_dir = run_root / f"evaluation_manual_stable_{ts}"
